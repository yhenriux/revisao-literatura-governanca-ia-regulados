...
const OUT_ROOT = path.resolve(process.env.V22_VISUAL_OUT || ROOT);
const VERSION = "v2.2";
...
const PNG_DIR = path.join(OUT_ROOT, "Recursos_do_artigo", VERSION, "imagens");
const SVG_DIR = path.join(OUT_ROOT, "Recursos_do_artigo", VERSION, "fontes_vetoriais");
...
function graph1(rows){
  const cats=categories(rows), vals=cats.map(c=>lookup(rows,c,"Studies")), colors=[C.dark,C.light,C.gray];
...
function graph2(rows){
  const cats=categories(rows), left=190,right=421,top=36,bottom=222,max=niceMax(Math.max(...rows.filter(r=>r.serie==="Total").map(r=>r.valor)));let b=axisGrid(left,right,top,bottom,ticksFor(max),max);b+=legend(270,16,[{shape:"circle",color:C.blue,label:"Total"},{shape:"diamond",color:C.orange,label:"Central evidence"}]);
  cats.forEach((cat,i)=>{const y=top+11+i*(bottom-top-20)/(cats.length-1),t=lookup(rows,cat,"Total"),c=lookup(rows,cat,"Central evidence"),xt=left+(right-left)*t/max,xc=left+(right-left)*c/max;b+=multiline(left-8,y+3,wrap(cat,28),8.5,"end");b+=line(xc,y,xt,y,C.pale,4.5);b+=circle(xt,y,4.1,C.blue);b+=diamond(xc,y,4.3,C.orange);b+=text(xt+6,y+3,String(t),8.5,"start",C.dark,700);b+=text(Math.max(xc-5,left),y-6,String(c),8.5,"end",C.orange,700);});
  b+=text((left+right)/2,244,"Number of studies",8.5,"middle");return svg(443,248,b);
}
function graph3(rows){
  const cats=categories(rows),left=112,right=420,top=35,bottom=173,max=niceMax(Math.max(...cats.map(c=>lookup(rows,c,"Central evidence")+lookup(rows,c,"Supporting evidence"))));let b=axisGrid(left,right,top,bottom,ticksFor(max),max);b+=legend(228,16,[{shape:"diamond",color:C.orange,label:"Central evidence"},{shape:"circle",color:C.light,label:"Supporting evidence"}]);
  cats.forEach((cat,i)=>{const y=top+12+i*27,c=lookup(rows,cat,"Central evidence"),s=lookup(rows,cat,"Supporting evidence"),wc=(right-left)*c/max,ws=(right-left)*s/max;b+=text(left-8,y+3,cat,8.5,"end");b+=rect(left,y-7,wc,14,C.orange);b+=rect(left+wc,y-7,ws,14,C.light);b+=text(left+wc/2,y+3,String(c),8.5,"middle",C.white,700);b+=text(left+wc+ws+6,y+3,String(c+s),8.5,"start",C.dark,700);});b+=text((left+right)/2,194,"Number of studies",8.5,"middle");return svg(443,198,b);
}
function graph4(rows){
  const cats=categories(rows),left=194,right=420,top=34,bottom=210,max=niceMax(Math.max(...rows.filter(r=>r.serie==="Total").map(r=>r.valor)));let b=axisGrid(left,right,top,bottom,ticksFor(max),max);b+=legend(270,16,[{shape:"circle",color:C.light,label:"Total"},{shape:"diamond",color:C.orange,label:"Central evidence"}]);
  cats.forEach((cat,i)=>{const y=top+10+i*22,t=lookup(rows,cat,"Total"),c=lookup(rows,cat,"Central evidence"),wt=(right-left)*t/max,wc=(right-left)*c/max;b+=multiline(left-8,y+3,wrap(cat,30),8.5,"end");b+=rect(left,y-6,wt,12,C.light);if(c)b+=rect(left,y-6,wc,12,C.orange);b+=text(left+wt+5,y+3,String(t),8.5,"start",C.dark,700);if(c)b+=text(left+wc/2,y+3,String(c),8.5,"middle",C.white,700);});b+=text((left+right)/2,231,"Number of studies",8.5,"middle");return svg(443,234,b);
}
function graph5(rows){
  const cats=categories(rows),left=205,right=420,top=36,bottom=169,max=niceMax(Math.max(...rows.filter(r=>r.serie==="Total").map(r=>r.valor)));let b=axisGrid(left,right,top,bottom,ticksFor(max),max);b+=legend(270,16,[{shape:"circle",color:C.blue,label:"Total"},{shape:"diamond",color:C.orange,label:"Central evidence"}]);
  cats.forEach((cat,i)=>{const y=top+12+i*29,t=lookup(rows,cat,"Total"),c=lookup(rows,cat,"Central evidence"),xt=left+(right-left)*t/max,xc=left+(right-left)*c/max;b+=multiline(left-8,y+3,wrap(cat,31),8.5,"end");b+=line(xc,y,xt,y,C.pale,5);b+=circle(xt,y,4.3,C.blue);b+=diamond(xc,y,4.3,C.orange);b+=text(xt+6,y+3,String(t),8.5,"start",C.dark,700);b+=text(xc,y-7,String(c),8.5,"middle",C.orange,700);});b+=text((left+right)/2,194,"Number of studies",8.5,"middle");return svg(443,198,b);
}
...
function graph6(rows){
  const cats=categories(rows),cols=["Technical","Interactional","Organizational","Regulatory","Evolutionary"],left=181,top=51,cw=48,ch=24,max=Math.max(...rows.map(r=>r.valor));let b="";
  ...
  b+=text(left+2.5*cw,253,"Number of studies by co-occurrence",8.5,"middle");return svg(443,256,b);
}
